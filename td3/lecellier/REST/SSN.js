...
var SSN = /** @class */ (function () {
    ...
    SSN.prototype.controlSsnKey = function () {
        let myValue = this.secu_number.substr(0, 13); // Extraction des numéros utiles au calcul
        let myNir = this.secu_number.substr(13); // Extraction des numéros utiles au calcul
        myValue.replace('2B', "18").replace("2A", "19"); // Remplacement des numeros speciaux par des chiffres definis
        let myNumber = +myValue; // Transformation en nombre 
        return (97 - (myNumber % 97) == +myNir); // calcul de la cle de controle
    };
    ...
    SSN.prototype.extractDateNaissance = function () {
        ...
        if (mois == 62 || mois == 63) { mois = 1;} // cas speciaux !
        let birth = new Date(this.secu_number.substr(1, 2), mois - 1); // Affiche le mois suivant sur l'appli, ajout de mois - 1 pour règler ce problème
        return birth;
    };
